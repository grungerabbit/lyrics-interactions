function Recipe($scope) {
	...
	$scope.storeFinal = function() {
		return ($scope.store.price / $scope.store.servings) * $scope.store.time * $scope.store.quality + $scope.store.special;
	}
	
	$scope.homeFinal = function() {
		return ($scope.home.price / $scope.home.servings) * $scope.home.time * $scope.home.quality + $scope.home.special;
	}
