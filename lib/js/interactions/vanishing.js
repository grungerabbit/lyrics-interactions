		...
		var VP = {
			...
			"makePolyhedron" : function (settings, paper) {
				...
				Vanishing.prototype.edge = function(start, end, thickness) {
					...
					this.seg(start, end).attr({"stroke-width": thickness});
				}
						...
						for (var u = 0; u < numScanlines; u++) {
							...
							*/				
							...
							if (orientation === "horizontal") {
								...
								// apparently this method only works for the middle line. which makes sense because we never define the other lines. shit.
								function adjustXvalues(segment, place) {
									...
									if ($scope.showVertices === true) {
										paper.circle(vertexX, vertexY, 3).attr({"fill": color});
									}
									return vertexX;
								} 
								
								self.edge(self.gc(adjustXvalues(firstLine, "first"), firstLine.adjust), self.gc(adjustXvalues(secondLine, "second"), secondLine.adjust), u + 3)
