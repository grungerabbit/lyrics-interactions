		...
		var VP = {
			...
			"makePolyhedron" : function (settings, paper) {
				...
				Vanishing.prototype.makeScanlines = function(numScanlines) {
					...
					/*
						if scanlines are vertical, we would want to know:
						
						PR and QS for spacing (distance / number of lines)
						RS and PQ for length (shit)
					
						let's start with PR and QS and spacing... with 8 lines.
					...
					*/
					...
					function findScanlineVertices(firstSeg, secondSeg, orientation) {
						var vertexListFirstSide = [];
						var vertexListSecondSide = [];
						...
						var firstSpaced = (((numScanlines - 1) / numScanlines) * firstSeg[0]);
						var firstOffset = (firstSeg[0] - firstSpaced);
						var firstDivisor = firstSpaced / numScanlines;
						
						
						var secondSpaced = (((numScanlines - 1) / numScanlines) * secondSeg[0]);
						var secondOffset = (secondSeg[0] - secondSpaced);
						var secondDivisor = secondSpaced / numScanlines;
						
						
						function getSpacers(segment, index) {
							this.space = (((numScanlines - 1) / numScanlines) * segment[index]);
							this.offset = segment[index] - this.space;
							this.divisor = this.space / numScanlines;
						}
						...
						for (var u = 0; u < numScanlines; u++) {
							...
							if (orientation === "horizontal") {
								var firstAdj = (firstDivisor * u) + (firstSeg[1].yCoord + firstOffset);
								var secondAdj = (secondDivisor * u) + (secondSeg[1].yCoord + secondOffset);
								
								/*
							
								console.log(self.getHypotenuse("three"))
								console.log(self.getSlope("three"))
															
								var cos = 1 / self.getHypotenuse("three");
								var sin = self.getSlope("three") / self.getHypotenuse("three");
															
								console.log(cos)
								console.log(sin)
															
								console.log(firstSeg[1].xCoord + (firstAdj * cos))
															
								*/
							
								
								function test(distance, selected, place) {
									
									if (place === "first") {
										var color = color ? color : "red";
										var hyp = self.getHypotenuse(selected);
										var cos = 1 / (self.hyp);
										var sin = self.slope / (self.hyp);
		                        	
										var adjX = distance * cos;
										var adjY = distance * sin;
										var vertexX, vertexY;
									
										console.log(u)
								
										vertexX = firstSeg[1].xCoord - adjX;
										vertexY = firstAdj;
										color = "green";
								...
								test(firstAdj, "three", "first")
								test(secondAdj, "three", "second")
								
								
								//sin = this.slope / (this.hyp)
							
								self.edge(self.gc(firstSeg[1].xCoord, firstAdj), self.gc(secondSeg[1].xCoord, secondAdj))
							} else if (orientation === "vertical") {
								/*
								var firstAdj = (firstDivisor * u) + (firstSeg[1].xCoord + firstOffset);
								var secondAdj = (secondDivisor * u) + (secondSeg[1].xCoord + secondOffset);
															
								self.edge(self.gc(firstAdj, firstSeg[1].xCoord), self.gc(secondAdj, secondSeg[1].xCoord))*/
