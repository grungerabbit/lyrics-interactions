...
vanish.controller('vanishingSettings', function vanishingSettings ($scope) {
	...
	$scope.scanlineUniform = true;
	$scope.scanlines = false;
	
	// {?} initial vanishing points
	$scope.horizon = 2800; // aka vpY1 and vpY2
	$scope.vpX1 = 40;
	$scope.vpX2 = 1900;
	$scope.vpX3 = 600;
	$scope.vpY3 = -10000;
	...
	$scope.addPrism = function() {
		...
		$scope.dataset.push(genericCube);
	}

	$scope.removePrism = function(index) {
		$scope.dataset.splice(index, 1);
	}

	var paper = new Raphael(0, 0, 4000, 4000);
	
	
	$scope.redraw = function() {
		paper.remove();
		paper = new Raphael(0, 0, 4000, 4000);
	...
	var VP = {
		"config" : {
			"paper": function () { return new Raphael(0, 0, 2000, 4000); },
			"gridColor": "#94e515"
		},
		"init" : function () {
		...
		"makePolyhedron" : function (settings, paper) {
			...
			var Vanishing = function(settings) {
				this.vpX1 = $scope.vpX1;
				this.vpY1 = $scope.horizon;
				this.vpX2 = $scope.vpX2;
				this.vpY2 = $scope.horizon;
				this.vpX3 = $scope.vpX3;
				this.vpY3 = $scope.vpY3;
				this.width = parseInt(settings.width, 10);
				this.height = parseInt(settings.height, 10);
				this.depth = parseInt(settings.depth, 10);
				this.seedX = parseInt(settings.seedX, 10);
				this.seedY = parseInt(settings.seedY, 10);
			};
