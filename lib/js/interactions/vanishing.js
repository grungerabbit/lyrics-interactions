	...
	vanish.controller('vanishingSettings', function vanishingSettings ($scope) {
		...
		var VP = {
			...
			"makePolyhedron" : function (settings, paper) {
				...
				Vanishing.prototype.makeScanlines = function() {
					...
					*/
					var self = this;
					function findSegmentPair(first, second) {
						var check1, check2;
						
						for (var b = 0; b < vertices.length; b++) {
							if (vertices[b].name === first) {
								check1 = vertices[b];
							} else if (vertices[b].name === second) {
								check2 = vertices[b];
							}
						}
						
						console.log("the length of segment " + first + second + " is " + self.getDistance(check1, check2));
					}
